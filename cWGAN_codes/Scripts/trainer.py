...
for i in range(PARAMS.n_epoch):
    ...
    # Saving intermediate output and checkpoint
    if (i+1) % PARAMS.save_freq == 0: 
        ...
        fig1.tight_layout()
        fig1.savefig(f"{savedir}/stats_val_sample_{i+1}.png")
        plt.close('all')

        torch.save(G_model.state_dict(),f"{savedir}/checkpoints/G_model_{i+1}.pth")
        torch.save(D_model.state_dict(),f"{savedir}/checkpoints/D_model_{i+1}.pth")
        torch.save(G_optim.state_dict(),f"{savedir}/checkpoints/G_optim_{i+1}.pth")
        torch.save(D_optim.state_dict(),f"{savedir}/checkpoints/D_optim_{i+1}.pth")
...
save_loss(mismatch_log,'mismatch',savedir,PARAMS.n_epoch)
