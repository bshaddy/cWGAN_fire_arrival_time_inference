...
from utils import *
from plot_dataset_results import plot_results
...
nimp_samples = 4 # Used only when z_dim > 0
...
for n in range(n_test):
    ...
    if PARAMS.z_dim is not None:
        ...
        save_data = np.stack((test_x.detach().numpy().squeeze(),
                              test_y.detach().numpy().squeeze(),
                              sample_mean,
                              sample_std),axis=0)
